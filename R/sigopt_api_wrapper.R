...
sigopt_GET <- function(path, query = NULL, ..., api_token = sigopt_api_token()) {
  ...
  req <- httr::GET("https://api.sigopt.com", path = path, query = query, auth, httr::user_agent("SigOptR/0.0.2"), ...)
  sigopt_check(req)
...
sigopt_POST <- function(path, body, ..., api_token = sigopt_api_token()) {
  ...
  req <- httr::POST("https://api.sigopt.com", path = path, body = body_json, encode = "json", auth, httr::user_agent("SigOptR/0.0.2"), ...)
  sigopt_check(req)
